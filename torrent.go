...
func NewTorrentSession(torrent string) (ts *TorrentSession, err error) {
	...
	////todo: edit t.totalSize, t.m.Info.PieceLength and t.m.Announce(tracker ip)
...
func (t *TorrentSession) getSortedInterestedPeers() (vec []*peerState) {
	if t.isSeeding() {
			...
			if t.PeerExist(peer.address) { //peer may already being closed
				...
				if err2 != nil {
					if err2 != io.EOF {
						...
						log.Println("Closing peer", peer.address, "because", "io.EOF")
					}
					t.ClosePeer(peer)
				}
			} else {
...
func (t *TorrentSession) IsActivePiece(piece int) (exist bool) {
	_, exist = t.activePieces[piece]
	return
}
...
func (t *TorrentSession) ChoosePiece(p *peerState) (piece int) {
	...
	if p.isSeed {
		log.Printf("total %v choose piece %v, from %v\n", index, piece, p.address)
	} else {
		log.Printf("total %v choose piece %v, from %v\n", index, piece, p.address)
	}
...
func (t *TorrentSession) DoMessage(p *peerState, message []byte) (err error) {
	...
	if len(p.id) == 0 {
		...
		messageId := message[0]
		//log.Println(messageId, "from", p.address)
		...
		case REQUEST:
			...
			//log.Println("request from", p.address, "piece", index)
			...
			if int64(begin)+int64(length) > t.m.Info.PieceLength {
				log.Println("begin", begin, "length", length, "PieceLength", t.m.Info.PieceLength)
		...
		case PIECE:
			...
			//log.Println("piece", index, "length", length, "from", p.address)
			if index >= uint32(p.have.n) {
				return errors.New("piece out of range.")
			}
...
func (t *TorrentSession) SaveResumeFile() {
	if f, err := os.OpenFile(t.fastResumeFile, os.O_CREATE|os.O_RDWR|O_BINARY, 0); err != nil {
		log.Println("xxx can not open fast resume file", t.fastResumeFile)
	} else {
		defer f.Close()
